...
float3 ApplyBloomThreshold(float3 color)
{
    ...
    float softness = min(max(0.0, brightness + _BloomThreshold.y), _BloomThreshold.z);
    softness = (softness * softness) * _BloomThreshold.w;
    float multiplier = max(softness, brightness - _BloomThreshold.x) / max(brightness, 1e-4);
